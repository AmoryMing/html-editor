...
冻结（动态 → 静态）· 点选 / 父级扩选 · 双击改字（contenteditable）· 跨文件复制粘贴（替换 / 前插 / 后插）· iframe srcdoc 内部编辑 · 删除 / 排序 · 插入文字段落 / 剪贴板截图（data-URL 内嵌）· **标注层（标记 / 隐藏预览 / 交付另存一键剥离）** · 元素级评论（角标钉 + 面板 + 署名 + 自动同步）· 撤销 30 步 · 保存自动备份 · mtime 冲突锁 · 局域网多人共编 · 程序化 API（`window.__hx`，可被 Claude Code 等 AI 工具驱动）
